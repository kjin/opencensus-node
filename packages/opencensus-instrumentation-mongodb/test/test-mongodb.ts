...
import {CoreTracer, RootSpan, SpanEventListener, SpanData} from '@opencensus/core';
...
function assertSpan(
  ...
  assert.strictEqual(
      rootSpanVerifier.endedSpans[0].name, expectedName);
  assert.strictEqual(
      rootSpanVerifier.endedSpans[0].kind, expectedKind);
}
